...
```
[ 村 ] 06:00 Hub（行動 2 回まで時間コスト 0）
   |
 Act1  2 ステップ   ← 森 / 墓地 / 街道 から毎ステップ 2〜3 択
   |
 Act2  3 ステップ   ← 城下町 / 地下水路 / 教会
   |
 Act3  2 ステップ   ← 城
   |
[ BOSS ] 24:00 または城最終ノード到達で強制発生
```

1 ステップ = 2h（森は K001 で 1h、水路→城 ショートカットは K008 で 1 ステップ短縮）。
**06:00 + 7 ステップ × 2h = 20:00**。余った 4h が「寄り道できる予算」であり、
ここを削ってでも Knowledge を取りに行くかがリスク判断になる。

### 13.2 ノード種別

| 種別 | 内容 | 時間 |
|---|---|---|
| Combat | 通常戦闘 → Gold + Skill 3 択 | 2h |
| Elite | 強敵 → Gold + レア Skill 3 択 + Item | 2h |
| Social | NPC との対話。Knowledge / Item / ルート解放 | 2h |
| Discovery | 探索。Knowledge 確定入手 | 2h |
| Shop | 購入・売却。Thief's Grace で盗める | 1h |
| Shrine | 高リスク高報酬（HP を払って Skill / Knowledge） | 1h |
| Rest | HP 40% 回復 or Skill 強化 | 3h |
| Ash Door | **灰の扉**。入室時に現在 HP の 25% を失う代わりに Knowledge 確定 + レア報酬 | 2h |

### 13.3 毎 Run 変わるもの / 変わらないもの

| 変わる（seeded random） | 変わらない（World Truth / 骨格） |
|---|---|
| ノード種別の配置、敵編成、Drop、Skill 3 択、Item レアリティ、危険度、Shop 在庫、どの Knowledge がどこに落ちるか | 幕構成、地名、NPC の正体、真犯人、ループの原因、Ending 条件 |

「マップはランダム、真実は不変」。これが AI 任せのミステリーにしないための線引き。
...
| M2 | **RunEngine**（マップ生成・時計・ノード解決・報酬 3 択） | ヘッドレスで 1 Run が完走する |
